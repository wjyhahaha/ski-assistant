---
name: ski-assistant
description: 全球滑雪综合服务助手（残票/低价票、早鸟预售、出行攻略、AI电子教练、智能雪场推荐、联网实时查价）。触发词：滑雪、雪票、雪场、外滑、早鸟票、残票、低价票、滑雪攻略、滑雪动作分析、推荐雪场、雪场天气、查价、滑雪预算。
---
...
2. 从 [references/resorts-reference.md](references/resorts-reference.md) 匹配2-3个雪场
3. WebSearch 补充交通、住宿、天气信息
4. 按 [references/travel-guide.md](references/travel-guide.md) 模板输出攻略，预算用 `python scripts/resort_recommender.py costs`
...
```bash
python scripts/ski_coach.py analyze  '<json>'  # 视觉分析（image必填，resort/run/difficulty可选）
python scripts/ski_coach.py analyze-batch '<json>'  # 批量分析（images列表或dir目录）
python scripts/ski_coach.py record   '<json>'  # 记录分析结果
python scripts/ski_coach.py history  [json]    # 历史记录（可过滤）
...
python scripts/ski_coach.py import   <path>    # 从导出文件恢复数据
```
...
# 推荐（先确认画像再推荐）
python scripts/resort_recommender.py recommend [json]   # 可选: include_weather/top_n/indoor_only

# 高山天气（按山顶海拔，最长16天）
python scripts/resort_recommender.py weather '<json>'   # resort + days

# 多雪场对比 / 费用估算
python scripts/resort_recommender.py compare '<json>'   # resorts + include_weather
python scripts/resort_recommender.py costs   '<json>'   # resort + days + people + from_city

# 数据库更新与发现
python scripts/resort_recommender.py update-db          # 从 GitHub 拉取最新数据
python scripts/resort_recommender.py discover '<json>'   # region(/enrich/merge/limit)
```
...
```bash
# 首选：flyai 直连查价（自动搜索机票+酒店+雪票）
python scripts/price_fetcher.py flyai-live '<json>'    # resort/from_city/date_start/date_end/people

# 备选：WebSearch 方案
python scripts/price_fetcher.py live-costs '<json>'    # 获取搜索策略（同上参数）
python scripts/price_fetcher.py search-queries '<json>' # 仅搜索关键词
python scripts/price_fetcher.py parse-results '<json>'  # 传入搜索结果生成报告
```

参数：`resort`（雪场名，支持模糊匹配）、`from_city`、`date_start/date_end`、`people`、`hotel_type`（可选）。parse-results 的 prices 中所有字段均可选，未提供的自动使用数据库参考值。
...
- **雪场参考**：[references/resorts-reference.md](references/resorts-reference.md) — 早鸟规律/联票体系/区域特色/速查表
- **攻略模板**：[references/travel-guide.md](references/travel-guide.md)
- **装备清单**：[scripts/gear_guide.py](scripts/gear_guide.py) — 按水平/地区/天数生成装备推荐
...
- **智能推荐**：[scripts/resort_recommender.py](scripts/resort_recommender.py) — 推荐/天气/对比/费用估算/联网发现
- **联网查价**：[scripts/price_fetcher.py](scripts/price_fetcher.py) — flyai 直连 + WebSearch 双模式
- **手动预算**：[scripts/budget_calculator.py](scripts/budget_calculator.py) — 用户已知费用明细时汇总
